...
export async function POST(request) {
  ...
  if (!verifyStripeSignature(rawBody, signature, secret)) return Response.json({ error: "Invalid webhook signature." }, { status: 400 });
  ...
  if (["checkout.session.completed", "checkout.session.async_payment_succeeded"].includes(event.type)) {
    ...
    console.log("Omni Terrain paid Checkout Session", JSON.stringify({
      event_id: event.id,
      session_id: session.id,
      store: session.metadata?.store || null,
