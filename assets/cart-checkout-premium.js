(() => {
  ...
  async function loadPriceMap() {
    try {
      ...
      console.warn("Omni Terrain cart pricing fallback", error);
      return {};
    }
  }

  function normalizedRows(cart, products, prices) {
    const rows = [];
    for (const item of cart) {
      const product = products.get(item.id);
      if (!product) continue;
      const price = prices[String(product.slug || "").split("/").pop()] || null;
      rows.push({item, product, priceCents:Number(price?.priceCents || 0)});
    }
    return rows;
  }
  ...
  function hero(active) {
    return `<section class="ot-commerce-hero"><div class="ot-commerce-kicker">Omni Terrain / US Store</div><h1>${active === 1 ? "Your cart." : "Secure checkout."}</h1><p>${active === 1 ? "Review your selected parts, quantities and merchandise subtotal before continuing." : "Review your order, then continue to Stripe for protected payment and delivery details when every selected item is eligible for online purchase."}</p>${steps(active)}</section>`;
  }
  ...
  function summary(rows, checkout=false) {
    ...
    return `<aside class="ot-commerce-card ot-summary"><h2>Order summary</h2>
      ...
      <div class="ot-summary-note">Final online purchase eligibility, stock and shipping are checked securely before Stripe accepts payment.</div>
      ${checkout ? "" : '<a class="ot-primary-btn" href="checkout.html">Continue to Secure Checkout →</a><a class="ot-secondary-btn" href="us-catalogue.html">Continue shopping</a>'}
  ...
  function renderCart(rows) {
    ...
    const items = rows.map(({item,product,priceCents}) => `<article class="ot-cart-item" data-cart-id="${esc(item.id)}">
      <div><div class="ot-cart-brand">${esc(product.brand || product.category || "Omni Terrain")}</div><h3>${esc(product.title || product.mpn || item.id)}</h3><div class="ot-cart-meta"><span>MPN ${esc(product.mpn || "—")}</span><span>${esc(product.category || product.segment || "Product")}</span></div></div>
      <div class="ot-cart-item-side"><div class="ot-line-price">${priceCents ? money(priceCents*item.quantity) : "Price unavailable"}</div>${priceCents ? `<div class="ot-unit-price">${money(priceCents)} each</div>` : ""}<div class="ot-qty-row"><div class="ot-qty"><button type="button" data-qty="down" aria-label="Decrease quantity">−</button><span>${item.quantity}</span><button type="button" data-qty="up" aria-label="Increase quantity">+</button></div><button class="ot-remove" type="button" data-remove>Remove</button></div></div>
    </article>`).join("");
  ...
  function rerender() {
    const cart = readCart();
    const rows = normalizedRows(cart,currentProducts,currentPrices);
  ...
  function checkoutLines(rows) {
    return rows.map(({item,product,priceCents})=>`<div class="ot-checkout-line"><div><b>${esc(product.title || product.mpn || item.id)}</b><span>${esc(product.brand || "")} · MPN ${esc(product.mpn || "—")} · Qty ${item.quantity}</span></div><strong>${priceCents ? money(priceCents*item.quantity) : "—"}</strong></div>`).join("");
  }
  ...
  function renderCheckout(rows) {
    ...
    main.innerHTML = `<div class="ot-commerce-wrap">${hero(2)}<div class="ot-commerce-grid"><section class="ot-commerce-card ot-checkout-panel"><h2>Secure payment</h2><p>Stripe collects payment, billing, phone and US delivery details on its protected checkout page.</p><div class="ot-payment-box"><h3>Ready to continue?</h3><p>Omni Terrain verifies each selected item against the server-side online-purchase gate before a payment session is created.</p><button class="ot-primary-btn" id="otSecurePay" type="button">Continue to Stripe →</button><div class="ot-payment-status" id="otPaymentStatus" role="status"></div></div><a class="ot-secondary-btn" href="cart.html">← Back to cart</a><div class="ot-pay-trust"><div><b>Card details</b>Never stored here</div><div><b>Delivery</b>US addresses</div><div><b>Payment</b>Server-verified pricing</div></div></section><aside class="ot-commerce-card ot-summary"><h2>Your order</h2><div class="ot-checkout-items">${checkoutLines(rows)}</div>${summaryInner(rows)}</aside></div></div>`;
    document.getElementById("otSecurePay")?.addEventListener("click",()=>startPayment(rows));
  ...
  function summaryInner(rows) {
    const subtotal=rows.reduce((sum,row)=>sum+(row.priceCents*row.item.quantity),0);
    return `<div class="ot-summary-row"><span>Merchandise</span><strong>${money(subtotal)}</strong></div><div class="ot-summary-row"><span>Shipping</span><strong>Confirmed at payment</strong></div><div class="ot-summary-row"><span>Tax</span><strong>Calculated if applicable</strong></div><div class="ot-summary-row total"><span>Subtotal</span><strong>${money(subtotal)}</strong></div><div class="ot-summary-note">Stripe checkout opens only when every selected item passes the current online-purchase eligibility checks.</div>`;
  }
  ...
  async function startPayment(rows) {
    ...
    button.disabled=true; button.innerHTML='<span class="ot-spinner"></span>Verifying order…';
    showStatus("Checking online purchase eligibility, stock and shipping before payment…","info");
    try {
      const response=await fetch("/api/us-create-checkout-session",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:rows.map(r=>({id:r.item.id,qty:r.item.quantity}))})});
      const data=await response.json().catch(()=>({}));
      if(!response.ok || !data.url) throw new Error(data.error || "Secure checkout is temporarily unavailable.");
      location.assign(data.url);
    } catch(error) {
      showStatus(`${esc(error.message || "Secure checkout is temporarily unavailable.")}<br><br><a href="cart.html">Review cart</a> · <a href="contact-and-order-help.html">Contact product support</a>`,"error");
  ...
  async function mount() {
    ...
    currentPrices=await loadPriceMap();
    rerender();
  }
