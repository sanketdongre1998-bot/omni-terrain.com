(() => {
  ...
  async function hydrateMarineLiveCards(grid,liveSlugs){
    ...
    const pageLinks=[...document.querySelectorAll('nav.pagination a[href]')].map(a=>a.getAttribute("href")||"").filter(href=>/^marine-\d+\.html(?:[?#].*)?$/i.test(href));
    if(!pageLinks.length)return;
    const existing=new Set([...grid.querySelectorAll(".card")].map(cardSlug));const additions=[];
    await Promise.all(pageLinks.map(async href=>{try{const response=await fetch(href,{cache:"force-cache"});if(!response.ok)return;const html=await response.text();const doc=new DOMParser().parseFromString(html,"text/html");[...doc.querySelectorAll("main .grid .card")].forEach(card=>{const slug=cardSlug(card);if(!slug||!liveSlugs.has(slug)||existing.has(slug))return;existing.add(slug);additions.push(card.cloneNode(true));});}catch(_){}}));
    additions.forEach(card=>grid.appendChild(card));const pagination=document.querySelector("nav.pagination");if(pagination)pagination.hidden=true;document.documentElement.classList.add("ot-marine-live-aggregated");
  }
  ...
  async function mount(){
    ...
    allCards.forEach(card=>{
      const live=liveSlugs.has(cardSlug(card));card.classList.toggle("ot-card-hidden",!live);card.setAttribute("aria-hidden",live?"false":"true");
    ...
    const input=controls.querySelector(".ot-search-input"),brandSel=controls.querySelector('[data-filter="brand"]'),priceSel=controls.querySelector('[data-filter="price"]'),sortSel=controls.querySelector('[data-filter="sort"]'),count=controls.querySelector("[data-visible-count]"),results=controls.querySelector(".ot-search-results"),moreBtn=more.querySelector("button");
    let displayLimit=PAGE_SIZE;

    function matches(card){
      const q=input.value.trim().toLowerCase(),brand=brandSel.value,range=priceSel.value;
      let ok=!q||haystack(card).includes(q);if(ok&&brand)ok=brandOf(card)===brand;
    ...
    function sortList(list){
      ...
      else sorted.sort((a,b)=>{const ai=featuredRank.get(cardSlug(a)),bi=featuredRank.get(cardSlug(b));if(ai!==undefined||bi!==undefined)return(ai===undefined?999:ai)-(bi===undefined?999:bi);return(defaultOrder.get(a)??9999)-(defaultOrder.get(b)??9999);});
      return sorted;
    }
    function apply(reset=false){
      if(reset)displayLimit=PAGE_SIZE;
      const matched=sortList(cards.filter(matches));
      allCards.forEach(card=>{card.classList.add("ot-card-hidden");card.setAttribute("aria-hidden","true");});
      matched.forEach((card,index)=>{grid.appendChild(card);const show=index<displayLimit;card.classList.toggle("ot-card-hidden",!show);card.setAttribute("aria-hidden",show?"false":"true");});
      const shown=Math.min(displayLimit,matched.length);count.textContent=matched.length>shown?`${shown} of ${matched.length}`:String(matched.length);
      more.hidden=shown>=matched.length;moreBtn.textContent=matched.length-shown>PAGE_SIZE?`Load ${PAGE_SIZE} more products`:`Load ${Math.max(0,matched.length-shown)} more products`;
    }
    ...
    input.addEventListener("input",()=>{apply(true);showSuggestions();});
    [brandSel,priceSel,sortSel].forEach(el=>el.addEventListener("change",()=>apply(true)));
    controls.querySelector(".ot-clear-filters").addEventListener("click",()=>{input.value="";brandSel.value="";priceSel.value="";sortSel.value="default";results.classList.remove("open");apply(true);input.focus();});
    moreBtn.addEventListener("click",()=>{displayLimit+=PAGE_SIZE;apply(false);});
    document.addEventListener("click",e=>{if(!controls.contains(e.target))results.classList.remove("open")});input.addEventListener("focus",showSuggestions);
    setTimeout(()=>{cards.forEach(decorateFeaturedCard);apply(true);},250);
  }
  ...
  function start(){if(file==="us-catalogue.html"&&!window.__OMNI_CATALOGUE_WIDE_READY__){document.addEventListener("omni:catalogue-ready",addAssets,{once:true});return;}addAssets();}
