(() => {
  ...
  function mountHomeBanner(){
    ...
    section.innerHTML='<div class="container"><div class="ot-growth-banner-inner"><div><div class="ot-growth-kicker">7 featured offers · Auto & truck</div><h2>Built for the next upgrade.</h2><p>Shop seven featured automotive offers with free standard shipping in the contiguous U.S. and secure online checkout.</p><div class="ot-growth-actions"><a class="ot-growth-cta" href="deals.html">Shop Featured Offers →</a><span class="ot-growth-code">Extra $5 off eligible $150+ regular-priced orders · Code <strong>OMNI5</strong></span></div></div><div class="ot-growth-benefits"><span>Clear online pricing</span><span>Secure online checkout</span><span>Free shipping on featured offers</span></div><div class="ot-growth-terms">OMNI5 excludes featured-offer items and cannot be combined with those shipping offers.</div></div></div>';
  ...
  async function mountDealsPage(){
    ...
    root.innerHTML=list.map((x,i)=>`<article class="ot-deal-card"><a class="ot-deal-media" href="${esc(x.p.slug||x.offer.slug)}"><span class="ot-deal-badge">★ Deal Unlocked</span><img src="${esc(imgs[i])}" alt="${esc(x.p.title||x.p.mpn)}" loading="${i<2?"eager":"lazy"}" decoding="async"></a><div class="ot-deal-body"><div class="ot-deal-brand"><span>${esc(x.p.brand||"Omni Terrain")}</span><span>MPN ${esc(x.p.mpn||"")}</span></div><h2>${esc(x.p.title||x.p.mpn)}</h2><p>${esc(descriptions[x.id]||"Featured product with clear product details and secure online checkout.")}</p><div class="ot-deal-price"><small>YOU SAVE ON SHIPPING · Free standard delivery</small>${money(x.row.priceCents)}</div><a class="ot-deal-button" href="${esc(x.p.slug||x.offer.slug)}">Claim This Deal →</a></div></article>`).join("");
  }
