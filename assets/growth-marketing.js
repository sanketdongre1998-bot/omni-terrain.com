(() => {
  ...
  const offerMath=row=>{const priceCents=Math.max(0,Math.round(Number(row?.priceCents)||0)),shippingValueCents=row?.shippingIncluded===true?Math.max(0,Math.round((Number(row?.shippingQuoteUSD)||0)*100)):0;return{priceCents,shippingValueCents};};
  ...
  function injectStyles(){
    ...
    const style=document.createElement("style");style.id="otGrowthMarketingStyles";style.textContent=`
      ...
      .ot-deals-page{padding:48px 0 82px}.ot-deals-hero{padding:44px;border-radius:25px;background:linear-gradient(125deg,#061629,#103958);color:#fff;box-shadow:0 22px 54px rgba(7,26,48,.18)}.ot-deals-hero h1{max-width:880px;margin:14px 0 12px;font:800 clamp(4rem,7vw,7.5rem)/.76 "Barlow Condensed",sans-serif;text-transform:uppercase;letter-spacing:-.04em}.ot-deals-hero p{max-width:760px;margin:0;color:#d4e0eb;font-size:14px;line-height:1.75}.ot-deals-chips{display:flex;flex-wrap:wrap;gap:8px;margin-top:22px}.ot-deals-chips span{padding:8px 10px;border:1px solid rgba(255,255,255,.16);border-radius:999px;font-size:10px;font-weight:800}.ot-deals-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr));gap:17px;margin-top:24px;max-width:100%;min-width:0}.ot-deal-card{display:flex;min-width:0;flex-direction:column;overflow:hidden;border:1px solid #dde4e9;border-radius:19px;background:#fff;box-shadow:0 11px 30px rgba(7,26,48,.05)}.ot-deal-media{position:relative;display:grid;place-items:center;height:250px;padding:24px;background:linear-gradient(145deg,#fafbfc,#e8edf0)}.ot-deal-media img{width:100%;height:190px;object-fit:contain}.ot-deal-badge{position:absolute;left:13px;top:13px;padding:7px 9px;border-radius:999px;background:linear-gradient(135deg,#ffd944,#f2b918);color:#071a30;box-shadow:0 8px 22px rgba(185,128,0,.22);font:900 10px/1 "DM Mono",monospace;text-transform:uppercase}.ot-deal-body{display:flex;min-width:0;flex:1;flex-direction:column;padding:19px}.ot-deal-brand{display:flex;flex-wrap:wrap;justify-content:space-between;gap:8px;color:#8b6a31;font:700 10px/1.3 "DM Mono",monospace;text-transform:uppercase}.ot-deal-card h2{overflow-wrap:anywhere;margin:11px 0 8px;color:#071a30;font:800 26px/.92 "Barlow Condensed",sans-serif;text-transform:uppercase}.ot-deal-card p{margin:0;color:#65717d;font-size:11px;line-height:1.6}.ot-deal-price{display:grid;gap:6px;margin-top:auto;padding-top:16px;color:#071a30}.ot-deal-value,.ot-deal-saving{display:flex;align-items:center;justify-content:space-between;gap:8px;color:#65717d;font-size:10px;font-weight:750}.ot-deal-saving{color:#167047}.ot-deal-saving strong{font-size:12px}.ot-deal-today small{display:block;margin-bottom:4px;color:#65717d;font:750 9px/1.3 "DM Mono",monospace;text-transform:uppercase}.ot-deal-today strong{font:900 30px/1 "Barlow Condensed",sans-serif}.ot-deal-button{display:flex;align-items:center;justify-content:center;margin-top:13px;min-height:48px;border-radius:9px;background:#071a30;color:#fff!important;text-decoration:none;font-weight:850}.ot-deals-info{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:22px}.ot-deals-info div{padding:18px;border:1px solid #e0e5e9;border-radius:15px;background:#fff}.ot-deals-info strong{display:block;color:#071a30;font-size:12px}.ot-deals-info span{display:block;margin-top:5px;color:#65717d;font-size:10px;line-height:1.5}html[data-ot-theme="dark"] .ot-deal-value,html[data-ot-theme="dark"] .ot-deal-today small{color:#aab8c5}html[data-ot-theme="dark"] .ot-deal-saving{color:#9fe0bc}html[data-ot-theme="dark"] .ot-deal-today strong{color:#f3f6fa}
  ...
  function mountHomeBanner(){
    ...
    section.innerHTML='<div class="container"><div class="ot-growth-banner-inner"><div><div class="ot-growth-kicker">Verified online inventory · Auto, truck &amp; specialist parts</div><h2>Find the right part. Know the price. Order with confidence.</h2><p>Search by brand or manufacturer part number, see clear online pricing, and get product support before checkout when fitment or application needs a second look.</p><div class="ot-growth-actions"><a class="ot-growth-cta" href="deals.html">Shop Featured Offers →</a><span class="ot-growth-code">Extra $5 off eligible $150+ regular-priced orders · Code <strong>OMNI5</strong></span></div></div><div class="ot-growth-benefits"><span>Verified online availability</span><span>Secure Stripe checkout</span><span>Free standard shipping on featured offers</span></div><div class="ot-growth-terms">OMNI5 excludes featured-offer items and cannot be combined with those shipping offers.</div></div></div>';
  ...
  async function mountDealsPage(){
    ...
    if(!list.length){root.innerHTML='<div class="ot-catalogue-safe-state"><strong>Featured offers are updating.</strong><span>Please refresh shortly or browse the online catalogue.</span><a href="us-catalogue.html">Shop online catalogue →</a></div>';return;}
    const imgs=await Promise.all(list.map(x=>productImage(x.p.slug||x.offer.slug)));
    root.innerHTML=list.map((x,i)=>{const pricing=offerMath(x.row),badge=pricing.shippingValueCents?"Free standard shipping":"Featured online price",breakdown=pricing.shippingValueCents?`<div class="ot-deal-value"><span>Online product price</span><span>${money(pricing.priceCents)}</span></div><div class="ot-deal-saving"><span>Standard U.S. shipping</span><strong>Included</strong></div><div class="ot-deal-today"><small>Price before tax</small><strong>${money(pricing.priceCents)}</strong></div>`:`<div class="ot-deal-today"><small>Online price before tax</small><strong>${money(pricing.priceCents)}</strong></div>`;return `<article class="ot-deal-card"><a class="ot-deal-media" href="${esc(x.p.slug||x.offer.slug)}"><span class="ot-deal-badge">★ ${badge}</span><img src="${esc(imgs[i])}" alt="${esc(x.p.title||x.p.mpn)}" loading="${i<2?"eager":"lazy"}" decoding="async"></a><div class="ot-deal-body"><div class="ot-deal-brand"><span>${esc(x.p.brand||"Omni Terrain")}</span><span>MPN ${esc(x.p.mpn||"")}</span></div><h2>${esc(x.p.title||x.p.mpn)}</h2><p>${esc(descriptions[x.id]||"Featured product with clear product details and secure online checkout.")}</p><div class="ot-deal-price">${breakdown}</div><a class="ot-deal-button" href="${esc(x.p.slug||x.offer.slug)}">View Product →</a></div></article>`;}).join("");
  }
  ...
  function mountPromoBox(){
    ...
    const firstCta=aside.querySelector(".ot-primary-btn,.checkout-note");if(firstCta?.parentNode)firstCta.parentNode.insertBefore(box,firstCta);else aside.appendChild(box);
  ...
  function mountCheckoutPromo(){
    ...
    const button=target.querySelector?.("#otSecurePay,.ot-primary-btn");if(button?.parentNode)button.parentNode.insertBefore(node,button);else target.appendChild(node);
  }
