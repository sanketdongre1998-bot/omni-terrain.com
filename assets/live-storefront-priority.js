(() => {
  ...
  async function mount(){
    ...
    if(grid){
      grid.innerHTML=featured.map((x,i)=>{const p=x.product,r=x.row,priceCents=Math.max(0,Math.round(Number(r?.priceCents)||0)),desc=marketingDescriptions[x.id]||String(p.description||"").split(". ")[0].slice(0,150)||"Shop online with clear pricing and product details.",badge=r.shippingIncluded===true?"Free standard shipping":"Featured product",priceNote=r.shippingIncluded===true?"Omni Terrain price · standard US shipping included":"Current Omni Terrain price";return `<article class="live-card" data-live-id="${esc(x.id)}"><a class="live-media" href="${esc(p.slug)}"><span class="live-badge">${badge}</span><img src="${esc(images[i])}" alt="${esc(p.title||p.mpn)}" decoding="async" loading="${i===0?"eager":"lazy"}"></a><div class="live-body"><div class="live-brand"><span>${esc(p.brand||"Omni Terrain")}</span><span>${esc(p.mpn||"")}</span></div><h3>${esc(p.title||p.mpn)}</h3><p>${esc(desc)}</p><div class="live-card-footer"><div class="live-offer-price"><div class="live-offer-today"><small>${priceNote}</small><strong>${money(priceCents)}</strong></div></div><a class="live-link" href="${esc(p.slug)}">View product →</a></div></div></article>`;}).join("");
    ...
    const launch=document.querySelector(".launch-strip .container span:last-child");if(launch)launch.textContent="7 featured products · Free standard shipping in the contiguous U.S.";
  }
