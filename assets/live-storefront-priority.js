(() => {
  ...
  async function mount(){
    ...
    if(grid){
      grid.innerHTML=featured.map((x,i)=>{const p=x.product,r=x.row,desc=marketingDescriptions[x.id]||String(p.description||"").split(". ")[0].slice(0,150)||"Shop online with clear pricing and product details.";return `<article class="live-card" data-live-id="${esc(x.id)}"><a class="live-media" href="${esc(p.slug)}"><span class="live-badge">Featured offer</span><img src="${esc(images[i])}" alt="${esc(p.title||p.mpn)}" decoding="async" loading="${i===0?"eager":"lazy"}"></a><div class="live-body"><div class="live-brand"><span>${esc(p.brand||"Omni Terrain")}</span><span>${esc(p.mpn||"")}</span></div><h3>${esc(p.title||p.mpn)}</h3><p>${esc(desc)}</p><div class="live-card-footer"><div class="live-price"><small>Free standard shipping</small>${money(r.priceCents)}</div><a class="live-link" href="${esc(p.slug)}">View offer →</a></div></div></article>`;}).join("");
