(() => {
  ...
  const addScript = (id, src, type = "text/javascript") => {
    if (document.getElementById(id)) return;
    const script = document.createElement("script");
    script.id = id;
    script.src = src;
    script.type = type;
    if (type !== "module") script.defer = true;
  ...
  /*
   * Homepage stability path.
   * The approved home is rendered by reference-storefront.js. Historically the
   * generic marketing/layout scripts also mutated the same DOM after render,
   * creating visible jumps, duplicate cleanup passes and inconsistent mobile UI.
   * Keep the home on one renderer and only load the assets it actually needs.
   */
  if (home) {
    document.documentElement.classList.add("ot-home-retail-v2");
    addCss("otReferenceStorefrontCss", "/assets/reference-storefront.css?v=4");
    addCss("otReferenceStorefrontFidelityCss", "/assets/reference-storefront-fidelity.css?v=2");
    addCss("otUsedOemIntegrationCss", "/assets/used-oem-integration.css?v=1");
  ...
  if (isUk) {
    ...
    addCss("otUkRefreshCss", "/assets/uk-storefront-refresh.css?v=retail-4");
  }
  ...
  if (file === "us-catalogue.html") {
    addCss("otCataloguePremiumCss", "/assets/catalogue-premium.css?v=retail-3");
    addScript("otCataloguePremiumJs", "/assets/catalogue-premium.js?v=2");
    addScript("otCatalogueWideJs", "/assets/catalogue-wide.js?v=2");
    addScript("otCatalogueControlsJs", "/assets/catalogue-controls.js?v=11");
  }
  if (file === "cart.html" || file === "checkout.html") {
    addCss("otCartPremiumCss", "/assets/cart-checkout-premium.css?v=retail-3");
    addScript("otCartPremiumJs", "/assets/cart-checkout-premium.js?v=2");
  }
  ...
  function scrubPublicBusinessDetails() {
    ...
    document.querySelectorAll(".market-note,.legal-note,footer p").forEach(node => {
      ...
      if (isUk && ukSensitive.test(text)) {
        if (node.classList.contains("market-note")) node.textContent = "UK storefront · Omni Terrain";
        else node.remove();
      } else if (!isUk && usSensitive.test(text)) {
        if (node.classList.contains("market-note")) node.textContent = "U.S. storefront · Omni Terrain";
        else node.remove();
      }
    });
  ...
  function compactReferenceMobileNav() {
    ...
    [featured, support].forEach(node => {
      if (node && !node.dataset.otDesktopLabel) node.dataset.otDesktopLabel = node.textContent || "";
    });
  ...
  function stabilityPass() {
    ...
    if (file === "cart.html" || file === "checkout.html") {
      document.querySelectorAll(".mobile-store-bar").forEach(node => node.remove());
    }
  }
  ...
  ready(() => {
    ...
    document.querySelectorAll(".brand,.ot-site-brand").forEach(brand => {
      brand.querySelectorAll(".ot-brand-crest,.brand-badge,.brand-mark,.logo-badge,.logo-mark").forEach(n => n.remove());
      ...
      img.src = "/assets/omni-terrain-approved-gt.webp?v=1";
  ...
  setTimeout(() => {
    addScript("otCustomerCopyJs", "/assets/customer-marketing-copy.js?v=2");
    addScript("otGrowthJs", "/assets/growth-marketing.js?v=4");
    addScript("otSavingsJs", "/assets/offer-savings-copy.js?v=1");
    addScript("otAnalyticsJs", "/assets/analytics-events.js?v=2");
  }, 700);
  ...
  const tune = img => {
    ...
    img.decoding = "async";
    const priority = Boolean(img.closest(".product-visual,.ot-ref-hero")) || Boolean(file === "uk.html" && img.closest(".ot-ref-category,.ot-ref-product"));
    img.loading = priority ? "eager" : "lazy";
    try { img.fetchPriority = img.closest(".product-visual,.ot-ref-hero") ? "high" : priority ? "auto" : "low"; } catch (_) {}
  };
  ready(() => document.querySelectorAll("img").forEach(tune));

  /* One bounded observer is enough on non-home pages; avoid repeated full-DOM scrub loops. */
  ...
  setTimeout(stabilityPass, 350);
  setTimeout(stabilityPass, 1200);
})();
