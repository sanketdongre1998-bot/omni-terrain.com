(() => {
  ...
  function mountCheckout(box, id) {
    ...
    if (note) note.textContent = "Secure online checkout is available. Final product pricing is validated server-side before Stripe opens.";
  ...
  function mount(attempt = 0) {
    ...
    if (document.querySelector(".ot-live-buybox, .ot-online-buybox")) return;

    const box = document.querySelector(".ot-display-buybox");
    const id = productId();
    if (!box || !id) {
      if (attempt < 10) setTimeout(() => mount(attempt + 1), 180);
      return;
    }
    if (box.dataset.otUniversalCheckout) return;
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", () => setTimeout(() => mount(), 160), { once: true });
