...
async function fetchText(path) {
  const response = await fetch(`${STOREFRONT}${path}`, {
    headers: { "User-Agent": "Omni-Terrain-Checkout/2.0" },
    cache: "no-store",
  });
...
async function buildCatalogue() {
  const [productsSource, pricesSource] = await Promise.all([
  ...
  if (!byId.size) throw new Error("No priced US storefront products are available.");
  return byId;
}
...
export async function resolveUsCheckoutItems(items) {
  ...
  const rows = items.map((row) => {
    ...
    if (!product?.enabled || !product?.storefrontVerified || Number(product.priceCents) <= 0) {
