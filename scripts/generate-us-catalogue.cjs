...
function segmentLabel(segment) {
  return segment === "automotive" ? "Automotive & Towing" : segment === "rv" ? "RV & Overlanding" : "Marine";
}

function categoryArt(segment) {
  if (segment === "automotive") {
    return `<svg viewBox="0 0 320 170" aria-hidden="true" focusable="false"><path d="M34 113h17l17-39h92l30 39h61"/><path d="M66 74 91 49h62l35 25"/><circle cx="87" cy="117" r="19"/><circle cx="211" cy="117" r="19"/><path d="M251 112h31m-12-15 17 15-17 15"/><path class="accent" d="M26 137h260"/></svg>`;
  }
  if (segment === "rv") {
    return `<svg viewBox="0 0 320 170" aria-hidden="true" focusable="false"><path d="M43 107V55h194l32 27v25"/><path d="M61 73h45v25H61zm66 0h45v25h-45zm67 0h27v38h-27z"/><circle cx="86" cy="119" r="18"/><circle cx="233" cy="119" r="18"/><path class="accent" d="M27 139h264M250 55v52"/></svg>`;
  }
  return `<svg viewBox="0 0 320 170" aria-hidden="true" focusable="false"><path d="m58 92 24 29h145l39-48H112L92 48H70"/><path d="M123 72V42h51l28 30"/><path class="accent" d="M31 138c28-16 49 16 77 0s49 16 77 0 49 16 77 0M44 153c24-12 42 12 66 0s42 12 66 0 42 12 66 0"/></svg>`;
}

function placeholder(product, withStatus = true) {
  return `<div class="product-placeholder placeholder-${esc(product.segment)}" role="img" aria-label="Product image pending for ${esc(product.title)}">${withStatus ? '<span class="status-chip">Currently unavailable</span>' : ''}<span class="visual-index">${esc(segmentLabel(product.segment))}</span><div class="category-art">${categoryArt(product.segment)}</div><div class="placeholder-mark"><span class="placeholder-brand">${esc(product.brand)}</span><strong>${esc(product.mpn)}</strong><b>Product image pending</b><span>Official asset approval pending</span></div></div>`;
}

function directImageRecord(product) {
  const record = imageByProductId.get(product.id);
  return record && record.imageUrl && record.status.includes("DIRECT") ? record : null;
}

function productVisual(product, withStatus = true, context = "card") {
  const record = directImageRecord(product);
  if (!record) return placeholder(product, withStatus);
  const loading = context === "detail" ? "eager" : "lazy";
  const priority = context === "detail" ? ' fetchpriority="high"' : "";
  return `<div class="product-photo product-photo-${esc(context)}">${withStatus ? '<span class="status-chip">Currently unavailable</span>' : ''}<img class="product-image" src="${esc(record.imageUrl)}" alt="${esc(`${product.brand} ${product.title}, MPN ${product.mpn}`)}" loading="${loading}" decoding="async" referrerpolicy="no-referrer"${priority} onerror="this.hidden=true;this.parentElement.classList.add('image-load-failed')"><div class="image-fallback" aria-hidden="true"><b>Image temporarily unavailable</b><span>Verified product record remains available</span></div><span class="image-source-tag">Manufacturer-hosted preview</span></div>`;
}

function card(product, index) {
  const shortCategory = product.category.split(" / ")[1];
...
function cataloguePage() {
  ...
  const categories = [
    ["automotive", "Automotive & Towing", counts.automotive, "Battery care, braking and towing essentials"],
    ["rv", "RV & Overlanding", counts.rv, "Power, shore connection and sanitation"],
...
function productPage(product) {
  const specs = product.specs.map(([label, value]) => `<div class="spec-row"><span>${esc(label)}</span><strong>${esc(value)}</strong></div>`).join("");
