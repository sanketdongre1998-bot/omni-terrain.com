...
def head(title, description, canonical, robots, site_url, schema=None, image=""):
    schema_html = ""
    if schema:
        schema_json = json.dumps(schema, ensure_ascii=False).replace("<", "\\u003c")
        schema_html = f'<script type="application/ld+json">{schema_json}</script>'
    image_url = absolute(site_url, image) if image else f"{site_url.rstrip('/')}/assets/omni-terrain-emblem.webp"
    og_type = "product" if isinstance(schema, dict) and schema.get("@type") == "Product" else "website"
    return f"""<!doctype html><html lang="en-US"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="robots" content="{esc(robots)}"><meta name="description" content="{esc(description)}"><meta name="theme-color" content="#ffffff"><title>{esc(title)}</title><link rel="canonical" href="{esc(canonical)}"><link rel="alternate" hreflang="en-US" href="{esc(canonical)}"><link rel="alternate" hreflang="x-default" href="{esc(canonical)}"><link rel="alternate" type="text/plain" href="{esc(site_url.rstrip('/'))}/llms.txt" title="Omni Terrain LLM site guide"><link rel="sitemap" type="application/xml" href="{esc(site_url.rstrip('/'))}/sitemap.xml"><meta property="og:site_name" content="Omni Terrain"><meta property="og:locale" content="en_US"><meta property="og:type" content="{og_type}"><meta property="og:title" content="{esc(title)}"><meta property="og:description" content="{esc(description)}"><meta property="og:url" content="{esc(canonical)}"><meta property="og:image" content="{esc(image_url)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{esc(title)}"><meta name="twitter:description" content="{esc(description)}"><meta name="twitter:image" content="{esc(image_url)}"><link rel="stylesheet" href="assets/wave1-storefront.css">{schema_html}</head>"""
...
def footer():
    return """<footer class="footer"><div class="container"><div><strong>Omni Terrain</strong><p>Specialist parts and equipment for road, water and travel.</p></div><div><a href="shipping-delivery-policy.html">Shipping</a> · <a href="returns-refunds-policy.html">Returns</a> · <a href="contact-and-order-help.html">Contact</a></div></div></footer><script defer src="assets/theme-toggle.js?v=27"></script><script defer src="assets/storefront-performance.js?v=16"></script>"""
...
def catalogue_page(rows, robots, site_url):
    ...
    for cat in CATEGORY_ORDER:
        cards.append(f'<a class="category-card" href="{CATEGORY_SLUGS[cat]}.html"><b>{esc(CATEGORY_LABELS[cat])}</b><small>{counts[cat]} products</small></a>')
