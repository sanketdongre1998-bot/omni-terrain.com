...
def audit(args):
    ...
    if len(paid)>10: blockers.append(f"Paid Search must stay <=10 products for $50 test; found {len(paid)}")
    if paid and not subset(paid,merchant): blockers.append("Every paid Search launch SKU must also be Merchant/page-quality ready")
    if len(paid)<5: warnings.append(f"Only {len(paid)} paid-ready products; target 5-10 for first test")
    paid_rows=[]
    for r in paid:
    ...
    report={"ready":not blockers,"counts":{"launch":len(launch),"visible":len(visible),"checkout":len(checkout),
        "merchant":len(merchant),"paid_search":len(paid)},"categories":dict(cats),"blockers":blockers,"warnings":warnings,"paid_products":paid_rows}
    Path(args.output).write_text(json.dumps(report,indent=2),encoding="utf-8")
    print("=== OMNI TERRAIN AUG-21 LAUNCH READINESS ==="); print("COUNTS =",report["counts"]); print("CATEGORY =",dict(cats))
    print("STATUS =", "READY" if not blockers else "BLOCKED")
...
def self_test(tmp):
    ...
    a.merchant=str(base/"merchant.csv"); a.paid=str(base/"paid.csv"); a.output=str(base/"report.json"); a.strict=True
    audit(a); data=json.loads((base/"report.json").read_text()); assert data["ready"] and data["counts"]["paid_search"]==5
...
def main():
    ...
    p.add_argument("--strict",action="store_true"); p.add_argument("--self-test",action="store_true"); a=p.parse_args()
    if a.self_test:
        import tempfile
